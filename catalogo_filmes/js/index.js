...
const estrela = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill" viewBox="0 0 16 16">
...
const preencheFaixa = classificacao => {
    if(classificacao < 12){
        return `<span class="faixa" style="background-color: green">L</span>`;
    }
...
const descricaoOpinioes = opinioes => {
    let descs = '';
    for(op of opinioes){
        descs += `<li>${op.descricao}</li>`;
    }
...
window.onload = function(){
    ...
    for(obj of objFilmes){
       section.innerHTML += `<article class="card" id="cardFilme">
                ...
                <div class="filmeclas">
                    <h2 class="titulo">${obj.titulo}</h2>
                    ${preencheFaixa(obj.classificacao)}
                </div>
                <div class="infos">
                    <small class="genero">${preencheGeneros(obj.generos)} </small>
                    <strong>Elenco: </strong><small class="elenco">${preencheElenco(obj.elenco)} </small>
                    
                </div>
                <br>
                <strong>Resumo. </strong><p class="resumo">${(obj.resumo)}</p>

                <div class="avaliacoes">
                    <div class="avaliacao-estrela">
                        <h3>Avaliações</h3>
                        <span class="estrelas">
                            ${retornaQtdEstrelas(obj.opinioes)}
                        </span>
                    </div>
                    
                    <ul class="rating">
                        ${descricaoOpinioes(obj.opinioes)}
                    </ul>
                </div>
                </article>`;
    }
